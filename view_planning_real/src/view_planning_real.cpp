...
int main(int argc, char** argv) {
    ...
    while (ros::ok()) {
        ...
        if (loop) {
            ...
            // 填充关节角度数据（示例值）[7](@ref)
            cmd_msg.joint = {angle,-13/180.0*M_PI, 29/180.0*M_PI, -0.0, 15/180.0*M_PI, -0.0};  // 6轴机械臂典型位置
            cmd_msg.speed = 0.15;  
            rate.sleep(); // 避免CPU空转
            ...
            rm_msgs::MoveJ cmd_msg;
            // 填充关节角度数据（示例值）[7](@ref)
            cmd_msg.joint = {0, -13.0/180.0*M_PI, 29/180.0*M_PI, -0.0, 10/180.0*M_PI, -0.0};  // 6轴机械臂典型位置
            cmd_msg.speed = 0.1;  
            rate.sleep(); // 避免CPU空转
            movej_pub.publish(cmd_msg);
            rate.sleep(); // 避免CPU空转
