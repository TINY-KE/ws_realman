...
/**********************添加变量***********************/
    bool     en_flag[6];               //当前关节使能状态 ，1为上使能，0为掉使能
    float    joint_position[3];        //当前关节角度，精度0.001°
    float    joint_temperature[6];     //当前关节温度，精度0.001℃
    float    joint_voltage[6];         //当前关节电压，精度0.001V
...
bool movej_canfd_follow = false;
bool movep_canfd_follow = false;
bool frce_position_move_follow = false;
...
ros::Publisher pub_GetSixZeroForce, pub_ArmError, pub_SysError, pub_JointErrorCode;
...
int Arm_Port = 8080;                    //机械臂TCP端口地址
int Udp_Port = 0;                       //机械臂UDP端口地址
...
int Movep_CANFD(POSE pose)
{
    ...
    if(movep_canfd_follow == false)
    {
        cJSON_AddFalseToObject(root, "follow");
    }
    else if(movep_canfd_follow == true)
    {
...
/****************************************Udp的配置函数************************************/
int Udp_Set_Realtime_Push(uint16_t cycle, uint16_t port)
{
    cJSON *root;
    char *data;
    char buffer[200];
...
int Force_Position_Move_Pose_Cmd(byte mode, byte sensor, byte dir, int force, POSE pose)
{
    ...
    if(frce_position_move_follow == false)
    {
        cJSON_AddFalseToObject(root, "follow");
    }
    else if(frce_position_move_follow == true)
    {
        cJSON_AddTrueToObject(root, "follow");
    }
...
int ManualSetForcePose_Cmd(std::string pose, int *joint)
{
    cJSON *root, *array;
    char *data;
    char buffer[200];
...
int Movej_CANFD(float *joint)
{
    ...
    cJSON_AddItemToObject(root, "joint", array);
    if(movej_canfd_follow == false)
    {
        cJSON_AddFalseToObject(root, "follow");
    }
    else if(movej_canfd_follow == true)
    {
        cJSON_AddTrueToObject(root, "follow");
    }
...
result = cJSON_GetObjectItem(root, "waypoint");
    ...
    if (json_member != NULL && json_member->type == cJSON_Array)
    {
        ...
        if (size == 4)
        {
            ...
            udp_arm_pose.orientation.x = Udp_RM_Joint.joint_quat[0];
            udp_arm_pose.orientation.y = Udp_RM_Joint.joint_quat[1];
            udp_arm_pose.orientation.z = Udp_RM_Joint.joint_quat[2];
            udp_arm_pose.orientation.w = Udp_RM_Joint.joint_quat[3];
...
if(force_sensor == 0 || (force_sensor == 1))
{
    ...
    if(result!= NULL)
        {
            ...
            if (json_member != NULL && json_member->type == cJSON_Array)
            {
                ...
                if (size == 6)
                {
                    ...
                    pub_GetSixForce.publish(Udp_Six_Force);
                }
                else
                {
                    cJSON_Delete(root);
            ...
            if (json_member != NULL && json_member->type == cJSON_Array)
            {
                ...
                if (size == 6)
                {
                    ...
                    pub_GetSixZeroForce.publish(Udp_Six_Zero_Force);
                }
                else
                {
                    cJSON_Delete(root);
...
if(force_sensor == 0 || (force_sensor == 2))
{
...
int Parser_Msg(char *msg)
{
    ...
    // Update:2023-7-25 @HermanYe
    // Get controller version
    json_state = cJSON_GetObjectItem(root, "Ctrl_version");
    if(json_state != NULL)
    {   
        uint32_t ctrl_version_original = (uint32_t)json_state->valuedouble;
        std::stringstream ss;
        ss << std::hex << ctrl_version_original;
        std::string hex = ss.str();
        std::cout << "Hexadecimal Version: " << hex << std::endl;
        // 提取版本号
        std::string major_hex = hex.substr(hex.length() - 3, 1);
        std::string minor_hex = hex.substr(hex.length() - 2, 1);
        std::string patch_hex = hex.substr(hex.length() - 1, 1);

        unsigned int major_version, minor_version, patch_version;
        ss.str("");
        ss.clear();
        ss << std::hex << major_hex;
        ss >> major_version;
        ss.clear();

        ss << std::hex << minor_hex;
        ss >> minor_version;
        ss.clear();

        ss << std::hex << patch_hex;
        ss >> patch_version;

        std::cout << "Major version: " << major_version << std::endl;
        std::cout << "Minor version: " << minor_version << std::endl;
        std::cout << "Patch version: " << patch_version << std::endl;
        // 将版本号转换为字符串
        std::string version_str = "v" + std::to_string(major_version) + "." + std::to_string(minor_version) + "." + std::to_string(patch_version);
        std::cout << "Controller Version: " << version_str << std::endl;
