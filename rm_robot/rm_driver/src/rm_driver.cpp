...
/************************设置角度跟随的高低跟随 true:高跟随 false:低跟随************************************/
void Set_Movej_Canfd_Follow_callback(const std_msgs::Bool msg)
{
        movej_canfd_follow = msg.data;
        // ROS_INFO("movej_canfd_follow = %d",movej_canfd_follow);
}
/******************************************************************************************************/

/************************查询角度跟随的高低跟随 true:高跟随 false:低跟随************************************/
void Get_Movej_Canfd_Follow_callback(const std_msgs::Empty msg)
{
        if(movej_canfd_follow == true)
        ROS_INFO("Movej_Canfd Is High Follow!!!");
        else if(movej_canfd_follow == false)
        ROS_INFO("Movej_Canfd Is Low Follow!!!");
        // ROS_INFO("movej_canfd_follow = %d",movej_canfd_follow);
}
/******************************************************************************************************/

/************************设置姿态跟随的高低跟随 true:高跟随 false:低跟随************************************/
void Set_Movep_Canfd_Follow_callback(const std_msgs::Bool msg)
{
        movep_canfd_follow = msg.data;
        // ROS_INFO("movep_canfd_follow = %d",movep_canfd_follow);
}
/*******************************************************************************************************/

/************************查询姿态跟随的高低跟随 true:高跟随 false:低跟随************************************/
void Get_Movep_Canfd_Follow_callback(const std_msgs::Empty msg)
{
        if(movep_canfd_follow == true)
        ROS_INFO("Movep_Canfd Is High Follow!!!");
        else if(movep_canfd_follow == false)
        ROS_INFO("Movep_Canfd Is Low Follow!!!");
        // ROS_INFO("movej_canfd_follow = %d",movej_canfd_follow);
}
/*******************************************************************************************************/
...
void Set_Realtime_Push_callback(const rm_msgs::Set_Realtime_Push msg)
{
    ...
    res = Udp_Set_Realtime_Push(cycle, port);
    if(res == 0)
    {
...
int main(int argc, char **argv)
{
    ...
    /********************************************follow高低速设置********************************************/
    Set_Movej_Canfd_Follow = nh_.subscribe("/rm_driver/Set_Movej_Canfd_Follow", 10, Set_Movej_Canfd_Follow_callback);
    Set_Movep_Canfd_Follow = nh_.subscribe("/rm_driver/Set_Movep_Canfd_Follow", 10, Set_Movep_Canfd_Follow_callback);
    /********************************************follow高低速查询********************************************/
    Get_Movej_Canfd_Follow = nh_.subscribe("/rm_driver/Get_Movej_Canfd_Follow", 10, Get_Movej_Canfd_Follow_callback);
    Get_Movep_Canfd_Follow = nh_.subscribe("/rm_driver/Get_Movep_Canfd_Follow", 10, Get_Movep_Canfd_Follow_callback);
    ...
    /*****************************力传感器外力数据********************************************/
    pub_GetSixZeroForce = nh_.advertise<rm_msgs::Six_Force>("/rm_driver/GetSixZeroForce", 100);
            ...
            if ((temp[0] == 0x0A) && (buffer_cnt > 1))
            {
                ...
                if (msg == 0x0D)
                {
                    ...
                    case GET_REALTIME_PUSH:
                        ...
                        if(Udp_Port != Udp_Setting.udp_port)
                        {
                            Udp_Socket_Close();
                            Udp_Port = Udp_Setting.udp_port;
                            while (Udp_Socket_Start())
                            {
                                if(ctrl_flag)
                                { // my action when signal set it 1
                    ...
                    case FORCE_POSITION_STATE:
                        ...
                        break;
                    case GET_SIX_FORCE:
                        Six_Force.force_Fx = Udp_RM_Joint.six_force[0];
                        Six_Force.force_Fy = Udp_RM_Joint.six_force[1];
                        Six_Force.force_Fz = Udp_RM_Joint.six_force[2];
                        Six_Force.force_Mx = Udp_RM_Joint.six_force[3];
                        Six_Force.force_My = Udp_RM_Joint.six_force[4];
                        Six_Force.force_Mz = Udp_RM_Joint.six_force[5];
