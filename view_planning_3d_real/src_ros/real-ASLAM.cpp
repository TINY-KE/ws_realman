...
void mapCallback(const nav_msgs::OccupancyGrid::ConstPtr& msg) {
    map_data_ = *msg;
    map_received_ = true;
    ROS_INFO_STREAM("Received NEW map with resolution: " << msg->info.resolution 
                    << " m/pixel, size: " << msg->info.width 
                    << "x" << msg->info.height);
}
...
int main(int argc, char **argv) {
    ...
    //四、生成ViewPlanning
    // (1)获取栅格地图
    ros::Subscriber map_sub_ = nh.subscribe("/map", 1, mapCallback);
    // (2)视点规划
    ...
    while (ros::ok()) {
        ...
        double robot_pose_x = 0, robot_pose_y = 0;
        for(auto ob: objects) {
            if(ob->explored)
                continue;
        ...
        view_planning.planning_for_real(target_object, map_data_, robot_pose_x, robot_pose_y, arm_results, FootPrints, CameraLinkCandidates);  //计算FootPrints
        ...
        for (int i = 0; i < FootPrints.size(); i++) {
            ...
            setPose(nh, "rm_65", FootPrints[i].position.x, FootPrints[i].position.y, FootPrints[i].position.z,
            ...
            // // 设置机械臂末端位姿
            // // 将位姿转换为tf2::Transform
            // tf2::Transform tf_world_foot, tf_world_cam;
            // tf2::fromMsg(FootPrints[i], tf_world_foot);
            // tf2::fromMsg(CameraLinkCandidates[i], tf_world_cam);
            // std::cout << "FootPrints Position: [" 
            //         << FootPrints[i].position.x << ", " 
            //         << FootPrints[i].position.y << ", " 
            //         << FootPrints[i].position.z << "]" << std::endl;
            // std::cout << "CameraLinkCandidates Position: [" 
            //         << CameraLinkCandidates[i].position.x << ", " 
            //         << CameraLinkCandidates[i].position.y << ", " 
            //         << CameraLinkCandidates[i].position.z << "]" << std::endl;
            // // 计算相对变换：T_foot_cam = inv(T_world_foot) * T_world_cam
            // tf2::Transform tf_foot_cam = tf_world_foot.inverse() * tf_world_cam;

            // // 转换回geometry_msgs::Pose
            // geometry_msgs::Pose pose;
            // tf2::toMsg(tf_foot_cam, pose);
            auto pose = CameraLinkCandidates[i];
