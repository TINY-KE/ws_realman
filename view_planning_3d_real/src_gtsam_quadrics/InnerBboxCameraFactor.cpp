...
/* ************************************************************************* */
gtsam::Vector InnerBboxCameraFactor::getMeasureBounds(
  ...
  try {
    ...
    if (NUMERICAL_DERIVATIVE) {
      ...
      //     funPtr(boost::bind(&InnerBboxCameraFactor::evaluateError, this,
...
/* ************************************************************************* */
gtsam::Vector InnerBboxCameraFactor::getPredictedBounds(
    const gtsam::Pose3& pose,
    gtsam::OptionalMatrixType H1) const {
...
/* ************************************************************************* */
gtsam::Vector InnerBboxCameraFactor::evaluateError(
    const gtsam::Pose3& pose,
    gtsam::OptionalMatrixType H1) const {
  ...
  // Quadric_.print("[InnerBboxCameraFactor::evaluateError]: 椭球体信息：");
  ...
  try {
    ...
    gtsam::Vector4 error = computeBoundaryDifferences(predictedBounds.vector(), measured_.vector());
    ...
    if (NUMERICAL_DERIVATIVE) {
      ...
      if (H1) {
        ...
        *H1 = db_dC * dC_dx;
        ...
        std::cout<<" [InnerBboxCameraFactor::evaluateError debug] db_dC = "<<std::endl<<db_dC<<std::endl;
        std::cout<<" [InnerBboxCameraFactor::evaluateError debug] dC_dx = "<<std::endl<<dC_dx<<std::endl;
      }
...
/* ************************************************************************* */
void InnerBboxCameraFactor::print(const std::string& s,
                              const gtsam::KeyFormatter& keyFormatter) const {
  cout << s << "InnerBboxCameraFactor(" << keyFormatter(key1())  << endl;
...
/* ************************************************************************* */
bool InnerBboxCameraFactor::equals(const InnerBboxCameraFactor& other,
                               double tol) const {
